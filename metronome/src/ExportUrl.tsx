...
function ExportUrl() {
  ...
  const createTransferUrl = (songs: Song[]) => {
    const cleanedSongs = songs.map((song) => {
      return song.name + '|' + song.tempo + '|' + song.timeSignature
    })
  ...
  return (
    <NpLayout>
      ...
      {songs.map((song) => (
        <div className='flex w-full max-w-sm text-xs justify-start gap-1 items-center cursor-pointer' key={song.id}>
